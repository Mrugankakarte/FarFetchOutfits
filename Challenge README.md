...
### 1 - products.parquet (inside dataset.tar.gz)
A csv table composed by roughly 400,000 products with the following columns:
...
### 3 - manual_outfits.parquet (inside dataset.tar.gz)
Table with 300,000 outfits. Each row consists of a single outfit with the following
columns:
...
### 4 - Testing outfits (stage1.parquet and stage2.parquet)
[Stage1 Test Outfits](https://storage.googleapis.com/sigir-challenge/stage1.parquet)
...
stage2.parquet will only be released when stage 2 starts

### 5 - Ground truth
After the end of the challenge the expected answers for the missing item
will be become available for download. The format of validation (used in dev 
...
Download the "Dataset (outfits)" and "Dataset (images)" and unzip both in your local machine. To verify that all is well, you can run the in baseline and evaluation folders.
